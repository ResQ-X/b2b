...
//   service_time_type: "NOW" | "SCHEDULED" | string;
//   quantity: number;
//   note: string | null;
//   location: string;
...
//   pagination: {
//     total: number;
//     page: string; // NOTE: string in API
//     limit: string; // NOTE: string in API
//     totalPages: number;
//   };
//   metrics?: {
//     scheduled: number;
...
// const mapStatus = (s: ApiOrder["status"]): OrderStatus => {
//   switch (s) {
//     case "COMPLETED":
//       return "Completed";
//     case "IN_PROGRESS":
...
// const mapApiToOrder = (row: ApiOrder): Order => {
//   const vehicle = row.asset
//     ? row.asset.plate_number
//       ? `${row.asset.asset_name} (${row.asset.plate_number})`
...
//     costNaira: 0, // API doesn't send cost — compute here if/when you have pricing
//     status: mapStatus(row.status),
//     dateISO: row.date_time,
//   };
// };
...
//   // fetch page from the (correct) endpoint
...
//         "/fleet-service/get-fuel-service", // ✅ correct path
//         { params: { page: p, limit: PER_PAGE } }
//       );

//       // logs
//       console.log("[API] raw response:", data);
//       console.log("[API] raw orders:", data.data);

//       const mapped = data.data.map(mapApiToOrder);
//       console.log("[UI] mapped orders:", mapped);

//       setOrders(mapped);
//       setPage(Number(data.pagination.page) || p);
...
//       // after creating, refresh list (first page to show most recent if that's your policy)
//       fetchPage(1);
//     } catch (error: any) {
...
//       {/* Loading / error */}
//       {loading && (
//         <div className="px-4 py-3 text-white/80 text-sm">Loading orders…</div>
//       )}
//       {err && <div className="px-4 py-3 text-red-300 text-sm">{err}</div>}

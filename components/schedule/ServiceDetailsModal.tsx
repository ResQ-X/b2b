...
import { Fuel, Wrench, AlertTriangle, Truck, Calendar, MapPin, FileText, DollarSign } from "lucide-react";
...
type ServiceDetails = {
  ...
  to_location?: string;
  to_location_longitude?: string;
  to_location_latitude?: string;
...
export default function ServiceDetailsModal({
  ...
  const ServiceIcon = getServiceIcon(service.service_category);
  const isFuel = service.service_category === "FUEL";
  const isMaintenance = service.service_category === "MAINTENANCE";
  const isEmergency = service.service_category === "EMERGENCY";
        ...
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-lg bg-[#FF8500]/20">
              <ServiceIcon className="h-6 w-6 text-[#FF8500]" />
            </div>
            <div>
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="bg-[#2D2B29] rounded-xl p-4">
            ...
            <div className="space-y-1">
              {isFuel && (
                <>
                  <InfoRow label="Fuel Type" value={formatServiceType(service.fuel_type || "")} />
                  <InfoRow label="Quantity" value={`${service.quantity || 0} Liters`} />
                  <InfoRow label="Service Type" value={formatServiceType(service.service_time_type || "")} />
                </>
              )}
              {isMaintenance && (
                <InfoRow label="Maintenance Type" value={formatServiceType(service.maintenance_type || "")} />
              )}
              {isEmergency && (
                <>
                  <InfoRow label="Emergency Type" value={formatServiceType(service.emergency_type || "")} />
                  {isTowing && service.towing_method && (
                    <InfoRow label="Towing Method" value={formatServiceType(service.towing_method)} />
                  )}
                </>
              )}
            </div>
              ...
              {service.assets.map((asset, index) => (
                ...
                >
                  ...
                  <div className="text-right text-sm text-white/70">
                    ...
                    {asset.fuel_type && <div>{formatServiceType(asset.fuel_type)}</div>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#2D2B29] rounded-xl p-4">
            ...
            <div className="space-y-1">
              <InfoRow label={isTowing ? "Pickup Location" : "Service Location"} value={service.location} />
          ...
          <div className="bg-[#2D2B29] rounded-xl p-4">
            ...
            <div className="space-y-1">
              <InfoRow label="Scheduled Time" value={formatDateTime(service.date_time)} />
              <InfoRow label="Order Date" value={formatDateTime(service.order_date)} />
            </div>
          </div>
            ...
            <div className="bg-[#2D2B29] rounded-xl p-4">
              ...
              <div className="space-y-1">
                {service.total_service_charge && (
                  <InfoRow label="Service Charge" value={formatCurrency(service.total_service_charge)} />
                )}
                {service.total_delivery_charge && (
                  <InfoRow label="Delivery Charge" value={formatCurrency(service.total_delivery_charge)} />
                )}
